...
| API keys and sealed mandates    | `src/backend/modules/api/controllers/ApiKeyController.ts`, `src/backend/modules/api/keyScopes.ts` (scope enforcement), `src/backend/services/keys/KeyMandateService.ts` (TEE-bound budgets, BYO import) | `key_mandates` table, `tests/unit/KeyParadigm.test.ts`                      |
...
| Landing page and IA              | `src/frontend/src/components/landing/` and `src/frontend/src/app/page.tsx`                                                       | [UX/IA review](./UX_IA_REVIEW.md), `src/frontend/src/lib/nav-items.ts`      |
...
| Outcome sources and connectors | `src/backend/services/outcomes/` (source config + GitHub connector), mandate `outcomeSources` | `tests/unit/GitHubOutcomeConnector.test.ts`, [outcome evidence plan](./OUTCOME_EVIDENCE_PLAN.md) |
