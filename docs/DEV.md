...
- **Both spend paths are gated.** The gate keys on `decision.status === "approved"`, which both the plain policy path and the confidential (FHE) path produce (`OwsWalletPolicyEvaluator.evaluatePolicyChecks` maps an FHE `approve` to `approved`). So a confidential-policy spend that the FHE approves but that meets `approvalThreshold` is held for operator signing just like a plain spend — `intent.amount` is plaintext wei even in confidential mode. The demo eval-only confidential endpoint (`POST /api/spend/encrypted` with `agentId`/`policyId`/`amountUsd`) returns a JSON decision but broadcasts nothing; its own threshold is the sealed mandate's `approvalThresholdUsd` (FHE/TEE), not this gateway field.
- **Dashboard attention.** A threshold-held run is `paused_for_approval`, which `AuditLogService.mapCreRunToAuditLog` maps to `outcome: "held"`; the frontend normalizes that to the `heldCount` feeding `deriveWorkspaceState` → the `"attention"` state. The held reason is surfaced as `auditLog.metadata.holdReason` (`"threshold"`, `"signing_failed"`, …) so the audit / dashboard can distinguish a threshold-gated hold from a policy-review hold.

The threshold decision is a pure helper, `meetsApprovalThreshold(amountWei, threshold)` (`src/backend/services/blockchain/spendThreshold.ts`), unit-tested in isolation. Confidential-path coverage is pinned by `tests/unit/SpendThresholdGate.test.ts`; the audit-mapping / dashboard-attention link by `tests/unit/AuditLogHoldReason.test.ts`.
