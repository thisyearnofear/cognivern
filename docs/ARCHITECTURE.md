...
Canton (Daml) is a swappable settlement backend for cognivern's sealed-bid vendor selection. The Fhenix-backed sealed-bid path holds bids as CoFHE ciphertext handles but can't complete the reveal; the Canton backend rewrites the settlement layer so the reveal actually works — atomically, in one transaction — while giving structural sub-transaction privacy that FHE alone can't. For HackCanton S2 the sealed-bid auction is the headline primitive; the agent-governance stack (Fhenix/Filecoin/0G/ChainGPT/X Layer) is supporting context. Value settlement is live on DevNet: a `PaymentDeposit` template is escrowed before the auction and atomically transferred to the winner inside `CloseAndReveal`. The updated DAR (package `d62e13ab…`) is uploaded to the shared DevNet participant, and `pnpm canton:proof` produces `valueSettledAtomically: true` with an on-ledger `settledAssetCid` — see `docs/CANTON.md` and `.artifacts/canton-devnet-proof-latest.json`. `SettlementProof.daml` passes all 9 assertions on the IDE ledger.
...
- **Migration note (iter 27, addressing iter 26 reviewer MAJOR):** `publishDeFiAction` dispatches to `xLayerDeFiVault` via Hyperlane ONLY when `plaintext == Approve (2)`. Deny/Hold resolutions emit `DeFiActionPublished` on the Fhenix chain but do NOT cross the Hyperlane bridge — this is intentional. Deny/Hold have nothing to execute on X Layer, so the unconditional every-resolution-dispatch legacy from the iter-25 synchronous baseline would be wasted Hyperlane gas.
