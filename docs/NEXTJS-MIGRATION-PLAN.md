...
export async function getAgentStatus(agentType: string) {
  const res = await fetch(`https://api.thisyearnofear.com/api/agents/${agentType}/status`, {
...
const FhenixProvider = dynamic(() => import('@/components/blockchain/FhenixProvider'), {
  ssr: false,
})
```
