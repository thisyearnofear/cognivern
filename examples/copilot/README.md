...
pnpm tsx examples/copilot/smoke-test.ts

# Interactive (asks the human to confirm at the execute step)
pnpm tsx examples/copilot/agent.ts "Pay 100 USDC to vendor 0xABC for API credits, only if their last audit was clean."
```
...
agent = Agent(
    ...
    model="gemini-3.1-pro-preview",
    instruction=open("./examples/copilot/instructions.md").read(),
    tools=[
...
```bash
pnpm tsx tooling/scripts/db/seed-hackathon.ts
```
...
5. **2:30–3:00** — Replay the same goal with a _bad_ vendor. Show the
