...
export class HyperlaneRelayerService {
  private destinations: Map<string, { wallet: ethers.Wallet; contractAddress: string; name: string }> = new Map();
  ...
  constructor(destinations: Record<string, RelayDestination>) {
    for (const [key, dest] of Object.entries(destinations)) {
      ...
      this.destinations.set(key, { wallet, contractAddress: dest.contractAddress, name: dest.name });
    }
  }
  ...
  async relayMessage(
    ...
    const contract = new ethers.Contract(dest.contractAddress, abi, dest.wallet);
    ...
    try {
      ...
      logger.info(`Relay tx confirmed on ${dest.name} in block ${receipt.blockNumber}`);
   ...
   */
  async relayToXLayer(originDomain: number, senderAddress: string, payload: string): Promise<string> {
    return this.relayMessage("xlayer", originDomain, senderAddress, payload);
  }

  async relayToMantle(originDomain: number, senderAddress: string, payload: string): Promise<string> {
