...
export class AutomatedForecastingService {
  ...
  async fetchOptimalCondition(): Promise<MarketCondition | null> {
    ...
    try {
      ...
      const candidates = conditions.filter(
        (c) => !this.forecastedMarkets.has(c.id),
      );
  ...
  async generateForecast(question: string): Promise<ForecastResult> {
    this.recordThought(
      `Generating forecast for: ${question.substring(0, 50)}...`,
    );
    ...
    try {
      ...
      logger.warn(
        "[ForecastingService] Market data enhancement failed:",
        error,
      );
    }
  ...
  async runForecastingCycle(): Promise<any> {
    ...
    try {
      ...
      if (!condition) {
        this.recordThought(
          "No suitable markets found. Waiting for next cycle.",
        );
      ...
      if (forecast.confidence >= 0.6) {
        try {
          ...
          if (marketPrice) {
            const edge = this.sapienceService.calculateEdge(
      ...
      this.forecastedMarkets.add(condition.id);
      this.recordThought(
        "Forecast successfully attested. Monitoring market updates.",
      );
      return {
        success: true,
        forecastTxHash,
  ...
  async getStats() {
    ...
    try {
      ...
      logger.warn(
        "[ForecastingService] Failed to get market stats for enhanced statistics:",
        error,
      );
    }
