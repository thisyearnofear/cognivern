...
export class AgentMetricsAggregator {
    ...
    tradingHistory;
    metricsService;
    agentsModule; // AgentsModule type
    constructor(tradingHistory, metricsService, agentsModule) {
     ...
     */
    async getComparisonMetrics(agentIds, filters) {
        const metrics = await Promise.all(agentIds.map((id) => this.getAgentMetrics(id)));
        return this.applyFilters(metrics.filter(Boolean), filters);
    }
    /**
    ...
    applyFilters(metrics, filters) {
        ...
        return metrics.filter((m) => {
            ...
            if (filters.ecosystems &&
                ...
                return false;
            if (filters.status && !filters.status.includes(m.status))
                return false;
            ...
            if (filters.minSharpeRatio !== undefined &&
                ...
                return false;
            if (filters.maxSharpeRatio !== undefined &&
    ...
    async fetchAgentMetrics(agentId) {
        try {
            ...
            return {
                ...
                status: agent.status,
                ecosystem: agent.ecosystem || "sapience",
    ...
    calculateTimeBasedPerformance(history, hoursAgo) {
        const cutoff = Date.now() - hoursAgo * 60 * 60 * 1000;
        const recentHistory = history.filter((d) => new Date(d.timestamp).getTime() > cutoff);
     ...
     */
    async getDashboardBundle(agentIds, filters) {
...
//# sourceMappingURL=AgentMetricsAggregator.js.map
