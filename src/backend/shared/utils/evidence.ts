...
export function extractCid(value: unknown): string | undefined {
  ...
  const cidPattern =
    /^(bafy[a-z0-9]+|baga[a-z0-9]+|Qm[1-9A-HJ-NP-Za-km-z]{44}|ipfs_[a-zA-Z0-9_-]+)$/;
...
export function enrichCreRunEvidence(run: CreRun): CreRun {
  const artifacts = run.artifacts.map((artifact) =>
    enrichArtifactEvidence(artifact),
  );
  ...
  return {
    ...
    evidence: {
      ...
      zeroGLocalHash: run.evidence?.zeroGLocalHash,
      filecoinCid: run.evidence?.filecoinCid,
      filecoinTxHash: run.evidence?.filecoinTxHash,
      filecoinActionId: run.evidence?.filecoinActionId,
