...
function getWorkspaceId(req: Request): string {
  return (
    (req.headers['x-workspace-id'] as string) || (req.query.workspaceId as string) || 'default'
  );
}

export function workspaceRateLimit(limit = DEFAULT_LIMIT, windowMs = DEFAULT_WINDOW_MS) {
  return async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    ...
    rateLimitStore.setWindow(key, entry, windowMs * 2).catch(() => {});
