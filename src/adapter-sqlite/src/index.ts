...
import { DatabaseAdapter, elizaLogger, type IDatabaseCacheAdapter } from '@elizaos/core';
import type {
  Account,
  Actor,
  GoalStatus,
  ...
  implements IDatabaseCacheAdapter
{
  ...
  async getParticipantUserState(roomId: UUID, userId: UUID): Promise<'FOLLOWED' | 'MUTED' | null> {
    ...
    const res = stmt.get(roomId, userId) as { userState: 'FOLLOWED' | 'MUTED' | null } | undefined;
  ...
  async logMemory(params: {
    ...
    this.db
      .prepare(sql)
      .run(v4(), params.userId, params.agentId, params.roomId, params.type, params.body);
  }
  ...
  async getGoals(params: {
    ...
    return goals.map((goal) => ({
      ...goal,
      objectives:
        typeof goal.objectives === 'string' ? JSON.parse(goal.objectives) : goal.objectives,
    }));
  }

  async updateGoal(goal: Goal): Promise<void> {
    const sql = 'UPDATE goals SET name = ?, status = ?, objectives = ? WHERE id = ?';
    this.db.prepare(sql).run(goal.name, goal.status, JSON.stringify(goal.objectives), goal.id);
  }
  ...
  async getRelationship(params: { userA: UUID; userB: UUID }): Promise<Relationship | null> {
    ...
    return (
      (this.db
        .prepare(sql)
        .get(params.userA, params.userB, params.userB, params.userA) as Relationship) || null
    );
  }
