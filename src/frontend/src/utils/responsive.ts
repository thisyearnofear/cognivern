...
export const responsive = {
  ...
  getValue: <T>(values: Partial<Record<keyof typeof designTokens.breakpoints, T>>): T | undefined => {
    const breakpoint = viewport.getCurrentBreakpoint();
    const breakpointOrder: (keyof typeof designTokens.breakpoints)[] = ['2xl', 'xl', 'lg', 'md', 'sm'];
  ...
  css: (property: string, values: Partial<Record<keyof typeof designTokens.breakpoints, string>>): string => {
    let css = '';
  ...
  gridColumns: (columns: Partial<Record<keyof typeof designTokens.breakpoints, number>>): string => {
    return responsive.css('grid-template-columns', 
      Object.fromEntries(
  ...
  spacing: (property: string, values: Partial<Record<keyof typeof designTokens.breakpoints, keyof typeof designTokens.spacing>>): string => {
    return responsive.css(property,
      Object.fromEntries(
        Object.entries(values).map(([bp, space]) => [bp, designTokens.spacing[space as keyof typeof designTokens.spacing]])
      )
    );
  },
};
...
export const layout = {
  ...
  getOptimalLayout: (contentMinWidth: number = 800) => {
    ...
    } else if (layout.shouldCollapseSidebar(contentMinWidth, viewport.getOptimalSidebarWidth('expanded'))) {
