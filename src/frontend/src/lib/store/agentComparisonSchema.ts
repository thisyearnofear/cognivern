...
export const field = {
  ...
  array: <T>(innerField: FilterField<T>, delimiter = ','): FilterField<T[]> => ({
    ...
    parse: (value) => {
      if (!value) return [];
      return value.split(delimiter).map(v => innerField.parse(v)).filter(v => v !== null) as T[];
    },
    serialize: (value) => {
      if (!value || value.length === 0) return null;
      return value.map(v => innerField.serialize(v as any)).filter(Boolean).join(delimiter);
    },
  }),
...
export const agentComparisonSchema = {
  ...
  // Sorting
  sortBy: field.stringLiteral(['winRate', 'totalReturn', 'sharpeRatio', 'totalTrades', 'avgLatency', 'agentName']),
