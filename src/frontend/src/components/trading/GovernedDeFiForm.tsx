      ...
      <CardContent>
        ...
        >
          ...
          <div css={infoBoxStyle}>
            <Lock size={14} />
            <span>
              Your budget is verified on Fhenix using FHE before the Vault executes this call on X
              Layer.
            </span>
          </div>
          ...
          {result && (
            <div css={resultStyle(result.success)}>
              {result.success ? (
                <>
                  <p>
                    <strong>Success!</strong> Decision dispatched via Hyperlane.
                  </p>
                  <p
                    css={css`
                      font-size: 0.75rem;
...
const resultStyle = (success: boolean) => css`
  ...
  font-size: 0.875rem;
  border: 1px solid
    ${success ? designTokens.colors.semantic.success[200] : designTokens.colors.semantic.error[200]};
`;
