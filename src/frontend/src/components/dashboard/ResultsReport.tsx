...
export default function ResultsReport() {
  ...
  const [reportInput, setReportInput] = useState<ReportInput>({
    companyName: '',
    industry: '',
    currentProcesses: ['Document Processing', 'Customer Support', 'Data Analysis'],
    employeeCost: 75000,
    taskVolume: 1000,
  ...
  const generateReport = () => {
    ...
    const { employeeCost, taskVolume, taskComplexity, currentProcesses } = reportInput;
    
    // Base calculations
    let timeReductionFactor = 0.65; // 65% time reduction
    let errorReductionFactor = 0.85; // 85% error reduction
    let costPerTask = (employeeCost / 2080) * (taskComplexity === 'low' ? 0.5 : taskComplexity === 'medium' ? 1 : 2);
    ...
    if (currentProcesses.includes('Document Processing')) {
      recommendations.push('Deploy document processing agents with OCR capabilities to maximize efficiency');
    }
    if (currentProcesses.includes('Customer Support')) {
      recommendations.push('Implement tiered support with agents handling Tier 1 inquiries and humans focusing on complex issues');
    }
    if (currentProcesses.includes('Data Analysis')) {
      recommendations.push('Use agents for data preparation and initial analysis, with human experts reviewing insights');
    }
    if (currentProcesses.includes('Compliance Monitoring')) {
      recommendations.push('Implement comprehensive governance policies to ensure all agent actions meet regulatory requirements');
    }
    
    // Add general recommendations
    recommendations.push('Start with a pilot program focused on highest-volume processes to demonstrate value');
    recommendations.push('Implement robust governance controls to maintain compliance and quality');
  ...
  return (
    <div className="results-report">
      <div className="report-header">
        ...
        <p>Generate a personalized report showing the potential impact of agent governance in your organization</p>
      </div>
              ...
              {processOptions.map((process) => (
                ...
                >
                  <div className="checkbox">
                    {reportInput.currentProcesses.includes(process) && <span className="checkmark">✓</span>}
                  </div>
                  <span>{process}</span>
                </div>
              ))}
            ...
            <div className="form-grid">
              <div className="form-group">
                ...
                <input 
                  ...
                  min="0"
                />
                <div className="input-hint">Include salary, benefits, training, and overhead</div>
              </div>
              ...
              <div className="form-group">
                ...
                <input 
                  ...
                  onChange={(e) => handleInputChange('taskVolume', Number(e.target.value))}
                  min="0"
                />
                <div className="input-hint">Number of tasks processed per month</div>
              </div>
              ...
              <div className="form-group">
                ...
                <div className="complexity-options">
                  <button 
                    className={reportInput.taskComplexity === 'low' ? 'selected' : ''}
                    onClick={() => handleInputChange('taskComplexity', 'low')}
                  >
                    Low
                  </button>
                  <button 
                    className={reportInput.taskComplexity === 'medium' ? 'selected' : ''}
                    onClick={() => handleInputChange('taskComplexity', 'medium')}
                  >
                    Medium
                  </button>
                  <button 
                    className={reportInput.taskComplexity === 'high' ? 'selected' : ''}
        ...
        <div className="report-step">
          ...
          <div className="report-document" ref={reportRef}>
            ...
            <div className="report-summary">
              ...
              <p>
                Based on your inputs, implementing agent governance for your {reportInput.currentProcesses.join(', ')} processes 
                could yield significant benefits. Our analysis shows potential for substantial cost savings, 
                efficiency improvements, and error reduction while maintaining full governance and compliance.
              </p>
            </div>
            ...
            <div className="report-highlights">
              <div className="highlight-card">
                ...
                <div className="highlight-value">${reportResult.annualSavings.toLocaleString()}</div>
              </div>
              <div className="highlight-card">
                <div className="highlight-title">Time Reduction</div>
                <div className="highlight-value">{reportResult.timeReduction.toFixed(0)}%</div>
              </div>
              <div className="highlight-card">
                <div className="highlight-title">Error Reduction</div>
                <div className="highlight-value">{reportResult.errorReduction.toFixed(0)}%</div>
              </div>
              <div className="highlight-card">
                <div className="highlight-title">3-Year ROI</div>
                <div className="highlight-value">{reportResult.roi.toFixed(0)}%</div>
              </div>
            </div>
            ...
            <div className="report-section">
              ...
              <div className="financial-details">
                <div className="financial-row">
                  ...
                  <div className="financial-value">{reportResult.paybackPeriod.toFixed(1)} months</div>
                </div>
                <div className="financial-row">
                  <div className="financial-label">Monthly Task Volume</div>
                  <div className="financial-value">{reportInput.taskVolume.toLocaleString()}</div>
                </div>
                <div className="financial-row">
                  <div className="financial-label">Current Process Cost (Annual)</div>
                  <div className="financial-value">
                    ${((reportInput.employeeCost / 2080) * 
                      (reportInput.taskComplexity === 'low' ? 0.5 : reportInput.taskComplexity === 'medium' ? 1 : 2) * 
                      reportInput.taskVolume * 12).toLocaleString()}
                  </div>
                </div>
                <div className="financial-row">
                  <div className="financial-label">Projected Agent Cost (Annual)</div>
                  <div className="financial-value">
            ...
            <div className="report-section">
              ...
              <ol className="next-steps-list">
                <li>Schedule a detailed assessment with our team to validate these projections</li>
                <li>Identify a pilot process to demonstrate value in your specific environment</li>
                <li>Develop a governance framework tailored to your industry requirements</li>
                <li>Create an implementation roadmap with clear milestones and success metrics</li>
              </ol>
            </div>
            
            <div className="report-footer">
              <p>This report was generated based on industry benchmarks and your provided information. 
              Actual results may vary based on specific implementation details and organizational factors.</p>
              <div className="report-contact">
                <p>For more information, contact us at <strong>sales@cognivern.com</strong></p>
              </div>
            </div>
          </div>
        </div>
