...
export default function EnhancedDashboard({ userType }: DashboardProps) {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      // Fetch system health
      try {
        const healthResponse = await fetch(getApiUrl("/api/system/health"), { headers });
      ...
      try {
        const agentsResponse = await fetch(getApiUrl("/api/agents/monitoring"), { headers });
  ...
  const generateMockRecommendations = (): AIRecommendation[] => [
    {
      ...
      description: 'Vincent agent response times are 40% higher than baseline. Consider implementing caching for sentiment analysis.',
      priority: 'medium',
    ...
    },
    {
      ...
      description: 'Vincent agent has attempted to access unapproved sentiment sources 3 times today.',
      priority: 'high',
    ...
    },
    {
      ...
      description: 'Market conditions suggest increasing trade frequency could improve returns by 8-12%.',
      priority: 'low',
  ...
  const renderSystemHealth = () => {
    ...
    return (
      <div css={systemHealthStyles}>
        <Card variant="default" css={css`${healthCardStyles}; &.${systemHealth.overall}`}>
          <CardContent>
            <CardTitle css={css`margin-bottom: ${designTokens.spacing[3]};`}>
              System Health
            </CardTitle>
            <div css={statusIndicatorStyles}>
              <span css={css`${statusDotStyles}; &.${systemHealth.overall}`} />
              <span css={css`font-weight: ${designTokens.typography.fontWeight.semibold}; text-transform: capitalize;`}>
        ...
        <Card variant="default" css={healthCardStyles}>
          <CardContent>
            ...
            <div css={css`font-size: ${designTokens.typography.fontSize['2xl']}; font-weight: ${designTokens.typography.fontWeight.bold}; color: ${designTokens.colors.primary[600]};`}>
              {systemHealth.metrics.activeAgents}/{systemHealth.metrics.totalAgents}
            </div>
            <div css={css`font-size: ${designTokens.typography.fontSize.sm}; color: ${designTokens.colors.neutral[600]};`}>
              Agents monitored
            </div>
        ...
        <Card variant="default" css={healthCardStyles}>
          <CardContent>
            ...
            <div css={css`font-size: ${designTokens.typography.fontSize['2xl']}; font-weight: ${designTokens.typography.fontWeight.bold}; color: ${designTokens.colors.semantic.success[600]};`}>
              {systemHealth.metrics.complianceRate}%
            </div>
            <div css={css`font-size: ${designTokens.typography.fontSize.sm}; color: ${designTokens.colors.neutral[600]};`}>
              Policy compliance
            </div>
        ...
        <Card variant="default" css={healthCardStyles}>
          <CardContent>
            ...
            <div css={css`font-size: ${designTokens.typography.fontSize['2xl']}; font-weight: ${designTokens.typography.fontWeight.bold}; color: ${designTokens.colors.primary[600]};`}>
              {systemHealth.metrics.totalActions.toLocaleString()}
            </div>
            <div css={css`font-size: ${designTokens.typography.fontSize.sm}; color: ${designTokens.colors.neutral[600]};`}>
              Actions logged
            </div>
  ...
  const renderAgentMonitoring = () => (
    <div>
      <h2 css={css`font-size: ${designTokens.typography.fontSize['2xl']}; font-weight: ${designTokens.typography.fontWeight.bold}; margin-bottom: ${designTokens.spacing[6]}; text-align: center;`}>
            ...
            <CardContent>
              <div css={agentHeaderStyles}>
                ...
                <div css={statusIndicatorStyles}>
                  ...
                  <span css={css`font-size: ${designTokens.typography.fontSize.sm}; font-weight: ${designTokens.typography.fontWeight.medium}; text-transform: capitalize;`}>
                    {agent.status}
                  </span>
                </div>
              </div>
              ...
              <div css={css`margin-bottom: ${designTokens.spacing[4]};`}>
                <h4 css={css`font-size: ${designTokens.typography.fontSize.sm}; font-weight: ${designTokens.typography.fontWeight.semibold}; margin-bottom: ${designTokens.spacing[3]};`}>
                  Performance Metrics
                </h4>
                <div css={metricsGridStyles}>
                  <div css={metricItemStyles}>
                    <div css={metricValueStyles}>{agent.performance.uptime}%</div>
                    <div css={metricLabelStyles}>Uptime</div>
                  </div>
                  <div css={metricItemStyles}>
                    <div css={metricValueStyles}>{agent.performance.successRate}%</div>
                    <div css={metricLabelStyles}>Success Rate</div>
                  </div>
                  <div css={metricItemStyles}>
                    <div css={metricValueStyles}>{agent.performance.avgResponseTime}ms</div>
                    <div css={metricLabelStyles}>Avg Response</div>
                  </div>
                  <div css={metricItemStyles}>
                    <div css={metricValueStyles}>{agent.performance.actionsToday}</div>
              ...
              <div css={css`margin-bottom: ${designTokens.spacing[4]};`}>
                ...
                <div css={metricsGridStyles}>
                  <div css={metricItemStyles}>
                    <div css={css`${metricValueStyles}; color: ${agent.riskMetrics.currentRiskScore > 0.5 ? designTokens.colors.semantic.warning[600] : designTokens.colors.semantic.success[600]};`}>
                      {(agent.riskMetrics.currentRiskScore * 100).toFixed(1)}%
                    </div>
                    <div css={metricLabelStyles}>Risk Score</div>
                  </div>
                  <div css={metricItemStyles}>
                    <div css={css`${metricValueStyles}; color: ${agent.riskMetrics.violationsToday > 0 ? designTokens.colors.semantic.error[600] : designTokens.colors.semantic.success[600]};`}>
                ...
                <div css={css`margin-top: ${designTokens.spacing[2]};`}>
                  ...
                  `}>
                    <div css={css`
                      height: 100%;
                      background: ${agent.riskMetrics.complianceRate > 95 ? designTokens.colors.semantic.success[500] : 
                                  agent.riskMetrics.complianceRate > 90 ? designTokens.colors.semantic.warning[500] : 
              ...
              {agent.financialMetrics && (
                <div css={css`margin-bottom: ${designTokens.spacing[4]};`}>
                  ...
                  <div css={metricsGridStyles}>
                    ...
                    <div css={metricItemStyles}>
                      <div css={css`${metricValueStyles}; color: ${agent.financialMetrics.dailyPnL >= 0 ? designTokens.colors.semantic.success[600] : designTokens.colors.semantic.error[600]};`}>
                        {agent.financialMetrics.dailyPnL >= 0 ? '+' : ''}${agent.financialMetrics.dailyPnL.toFixed(2)}
                      </div>
                      <div css={metricLabelStyles}>Daily P&L</div>
                    </div>
                  </div>
                  <div css={css`text-align: center; margin-top: ${designTokens.spacing[2]};`}>
                    <div css={metricValueStyles}>{agent.financialMetrics.winRate}%</div>
  ...
  const renderRecommendations = () => (
    <div>
      <h2 css={css`font-size: ${designTokens.typography.fontSize['2xl']}; font-weight: ${designTokens.typography.fontWeight.bold}; margin-bottom: ${designTokens.spacing[6]}; text-align: center; display: flex; align-items: center; justify-content: center; gap: ${designTokens.spacing[2]};`}>
        🤖 AI Recommendations
      ...
      <div css={recommendationsStyles}>
        {recommendations.map((rec) => (
          <Card key={rec.id} variant="default" css={css`${recommendationCardStyles}; &.${rec.type}`}>
            <CardContent>
              ...
              <div css={css`margin-bottom: ${designTokens.spacing[4]};`}>
                <div css={css`font-size: ${designTokens.typography.fontSize.sm}; color: ${designTokens.colors.neutral[600]}; margin-bottom: ${designTokens.spacing[1]};`}>
                  <strong>Impact:</strong> {rec.impact}
                </div>
                {rec.estimatedBenefit && (
                  <div css={css`font-size: ${designTokens.typography.fontSize.sm}; color: ${designTokens.colors.semantic.success[600]};`}>
    ...
    <div css={containerStyles}>
      <div css={heroStyles}>
        ...
        <p css={heroSubtitleStyles}>
          Monitor, control, and optimize your autonomous agents with enterprise-grade governance and real-time insights
        </p>
      </div>
      ...
      <div css={css`margin-top: ${designTokens.spacing[8]};`}>
        <BlockchainStatus />
      </div>
    </div>
  );
