...
const STATE_COLORS: Record<AgentState, { bg: string; text: string; glow: string }> = {
  ...
  BUSY: { bg: "bg-emerald-950", text: "text-emerald-400", glow: "shadow-[0_0_12px_rgba(34,197,94,0.3)]" },
          ...
          setCores((prev) =>
            prev.map((core, i) => {
              if (i === 2 && runningCount > 0) {
                return { ...core, state: "BUSY" as AgentState, task: `${runningCount} active run(s)`, lastActivity: "processing" };
              }
        ...
        <div className="space-y-0.5 max-h-32 overflow-y-auto scrollbar-thin">
          ...
          {cores
            ...
            .map((c) => (
              ...
              >
                <span className={STATE_COLORS[c.state].text}>
                  [{c.state}]
                </span>{" "}
