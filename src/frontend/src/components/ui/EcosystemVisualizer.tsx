...
const ViewToggle = styled.button<{ active: boolean }>`
  background: ${({ active }) => (active ? designTokens.colors.primary[500] : 'transparent')};
  border: 1px solid
    ${({ active }) =>
  ...
  const listColumns: Column<Node>[] = [
    {
      ...
      sortable: true,
      render: (val, record) => (
        <div style={{ fontWeight: 'bold', color: getNodeColor(record.status) }}>{val}</div>
      ),
    },
    {
      key: 'type',
    ...
    },
    {
      ...
      sortable: true,
      render: (val) => (
        <Badge variant={val === 'active' ? 'success' : val === 'warning' ? 'warning' : 'default'}>
          {val}
        </Badge>
      ),
    },
      ...
      {viewMode === 'spatial' ? (
        ...
        <div style={{ padding: '60px 20px 20px' }}>
          <DataTable
            ...
            pagination={
              processedNodes.length > 10
                ? { current: 1, pageSize: 10, total: processedNodes.length, onChange: () => {} }
                : undefined
            }
          />
        </div>
