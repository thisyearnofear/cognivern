...
const INTENT_CORE_MAP: Record<string, { coreIdx: number; state: AgentState }[]> = {
...
function applyIntentToCores(cores: CoreState[], intentType: string): CoreState[] {
  const activation = INTENT_CORE_MAP[intentType] || [{ coreIdx: 3, state: 'BUSY' }];
