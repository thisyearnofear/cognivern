...
export class SapienceService {
  ...
  constructor(config: Partial<SapienceConfig> = {}) {
    this.config = {
      ...
      etherealRpcUrl:
        config.etherealRpcUrl ||
        process.env.ETHEREAL_RPC_URL ||
    ...
    if (!this.config.privateKey) {
      logger.warn(
        "SapienceService initialized without private key. Operations will fail.",
      );
    }

    // Ethereal Provider for trading
    this.etherealProvider = new ethers.JsonRpcProvider(
      this.config.etherealRpcUrl,
    );
  ...
  async getMarketPrice(conditionId: string): Promise<MarketPrice | null> {
    try {
      ...
      const noOutcome = condition.outcomes?.find(
        (o: any) => o.name?.toLowerCase().includes("no") || o.name === "0",
      );
  ...
  async executeTrade(trade: TradeRequest): Promise<string> {
    try {
      ...
      const marketAddress =
        contracts.predictionMarket[CHAIN_ID_ETHEREAL]?.address;
      if (!marketAddress) {
        throw new Error(
          "Prediction market contract address not found for Ethereal chain",
        );
      }

      // Get the resolver address
      const resolverAddress = (trade.resolver ||
        contracts.umaResolver[CHAIN_ID_ETHEREAL]?.address) as `0x${string}`;
      if (!resolverAddress) {
      ...
      logger.info(
        `Collateral ready. WUSDe balance: ${preparation.wusdBalance.toString()}`,
      );
