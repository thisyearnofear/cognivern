...
export class MetricsService {
  ...
  async getMetrics(period: MetricsPeriod): Promise<Metrics> {
    ...
    const startTime = new Date(
      endTime.getTime() - this.getPeriodDuration(period)
    );
    ...
    try {
      ...
      for (let attempt = 1; attempt <= 3; attempt++) {
        try {
          ...
          logger.debug(
            `Found ${result.objects.length} metric objects in bucket`
          );

          if (result.objects.length === 0) {
            logger.info(
              "No metrics found in Recall bucket, returning empty metrics"
            );
      ...
      // Throw error instead of returning empty metrics
      throw new Error(
