...
export class GovernanceStorageService {
  ...
  constructor(recallClient?: RecallClient, bucketAddress?: Address) {
    if (recallClient && bucketAddress) {
      ...
      logger.info('GovernanceStorageService initialized with Recall integration');
    } else {
      logger.warn('GovernanceStorageService initialized without Recall client - will operate in simulation mode');
    }
   ...
   */
  initializeWithRecall(recallClient: RecallClient, bucketAddress: Address): void {
    this.recallService = new RecallService(recallClient, bucketAddress);
  ...
  async getOrCreateBucket(bucketName: string, config: BucketConfig): Promise<void> {
    try {
      if (this.recallService) {
        ...
        await this.recallService.storeObject(`buckets/${bucketName}`, 'config', config);
      }
  ...
  async addObject(bucketName: string, object: GovernanceObject): Promise<void> {
    try {
      ...
      if (bucketConfig.encryption && objectToStore.data) {
        objectToStore.data = await CryptoUtils.encrypt(objectToStore.data, 'default-encryption-key');
      }
      ...
      if (this.recallService) {
        ...
        await this.recallService.storeObject(bucketName, object.key, objectToStore);
      }
  ...
  async getObject(bucketName: string, key: string): Promise<GovernanceObject | null> {
    try {
      ...
      if (!this.recallService) {
        ...
        logger.debug(`Simulation mode: Object not found in bucket ${bucketName}`, { key });
        return null;
      }

      // Retrieve object from Recall with bucket prefix
      const object = await this.recallService.getObject<GovernanceObject>(bucketName, key);
      ...
      if (bucketConfig.encryption && object.data) {
        try {
          object.data = await CryptoUtils.decrypt(object.data, 'default-encryption-key');
        } catch (decryptError) {
          logger.warn(`Failed to decrypt object ${key} in bucket ${bucketName}`, {
  ...
  async listObjects(bucketName: string, prefix?: string): Promise<GovernanceObject[]> {
    try {
      ...
      if (!this.recallService) {
        ...
        logger.debug(`Simulation mode: No objects in bucket ${bucketName}`, { prefix });
        return [];
      }
      ...
      for (const objectKey of objectKeys) {
        try {
          ...
          const object = await this.recallService.getObject<GovernanceObject>(bucketName, actualKey);
          if (object) {
            object.key = actualKey;
            
            // Decrypt if needed (but might be expensive for listing)
            if (bucketConfig.encryption && object.data && typeof object.data === 'string') {
              try {
                object.data = await CryptoUtils.decrypt(object.data, 'default-encryption-key');
              } catch (decryptError) {
                // For listing, we can skip decryption errors and just log them
                logger.debug(`Skipping decryption for object ${actualKey} during listing`, {
                  error: decryptError instanceof Error ? decryptError.message : 'Unknown error'
                });
              }
            }
          ...
          logger.warn(`Failed to retrieve object ${objectKey} during listing`, {
            error: objectError instanceof Error ? objectError.message : 'Unknown error'
          });
          // Continue with other objects
        }
      }

      logger.debug(`Listed ${governanceObjects.length} objects from bucket ${bucketName}`, { 
        prefix,
  ...
  private async loadBucketConfig(bucketName: string): Promise<void> {
    ...
    try {
      const config = await this.recallService.getObject<BucketConfig>(`buckets/${bucketName}`, 'config');
  ...
  async getBucketStats(bucketName: string): Promise<{
    ...
    try {
      ...
      const lastModified = objects.length > 0 
        ? new Date(Math.max(...objects.map(obj => obj.timestamp))).toISOString()
        : null;
