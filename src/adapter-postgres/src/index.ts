  ...
  implements IDatabaseCacheAdapter
{
  ...
  async getAccountById(userId: UUID): Promise<Account | null> {
    return this.withDatabase(async () => {
      ...
      return {
        ...account,
        details:
          typeof account.details === 'string' ? JSON.parse(account.details) : account.details,
      };
    }, 'getAccountById');
  }
      ...
      elizaLogger.debug('Fetching memories:', {
        ...
        timeRange:
          params.start || params.end
            ? {
                start: params.start ? new Date(params.start).toISOString() : undefined,
  ...
  async removeGoal(goalId: UUID): Promise<void> {
    ...
    return this.withDatabase(async () => {
      try {
        const result = await this.pool.query('DELETE FROM goals WHERE id = $1 RETURNING id', [
          goalId,
        ]);
  ...
  async removeParticipant(userId: UUID, roomId: UUID): Promise<boolean> {
    return this.withDatabase(async () => {
      try {
        await this.pool.query(`DELETE FROM participants WHERE "userId" = $1 AND "roomId" = $2`, [
          userId,
          roomId,
        ]);
        return true;
  ...
  async getCache(params: { key: string; agentId: UUID }): Promise<string | undefined> {
    return this.withDatabase(async () => {
      try {
        ...
        const { rows } = await this.query<{ value: string }>(sql, [params.key, params.agentId]);
