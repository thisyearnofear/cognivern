...
function extractCitationLabels(run) {
    return (run.provenance?.citations || []).map((citation) => citation.label);
}
export function enrichArtifactEvidence(artifact) {
...
export function enrichRunEventEvidence(event, run) {
...
export function enrichCreRunEvidence(run) {
    ...
    const events = (baseRun.events || []).map((event) => enrichRunEventEvidence(event, baseRun));
    const runCid = artifacts.map((artifact) => artifact.evidence?.cid).find(Boolean) ||
