...
export function buildAuditEvidence(params: {
  ...
  policyChecks?: Array<{ policyId: string }>;
}): EvidenceEnvelope {
  const policyIds = (params.policyChecks || []).map((check) => check.policyId);
  const artifactIds = Array.isArray(params.details?.artifactIds)
    ? params.details.artifactIds.filter(
        (value: unknown): value is string => typeof value === "string",
      )
    : [];
  const citations = Array.isArray(params.details?.citations)
    ? params.details.citations.map((citation: any) =>
        typeof citation === "string" ? citation : citation?.label,
      ).filter((value: unknown): value is string => typeof value === "string")
    : [];
  const cid = extractCid(params.details);
