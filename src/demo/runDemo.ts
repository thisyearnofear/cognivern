...
async function runDemo() {
  ...
  const unauthorizedChecks =
    await policyService.evaluateAction(unauthorizedAction);
  const unauthorizedAllowed =
    await policyService.enforcePolicy(unauthorizedAction);
  await auditService.logAction(
    unauthorizedAction,
    unauthorizedChecks,
    unauthorizedAllowed
  );
  await metricsService.recordAction(
    unauthorizedAction,
    unauthorizedChecks,
    300
  );
  ...
  for (const action of highLoadActions) {
    ...
    if (!allowed) {
      console.log(
        "Rate limit kicked in after",
        action.metadata.requestsPerMinute,
        "requests"
      );
      break;
    }
