 ...
 *
 * Run with: pnpm tsx scripts/demo/test-keeperhub-rebalance.ts
 *
...
async function startMock(): Promise<void> {
  return new Promise((resolve, reject) => {
    mockProcess = spawn("node", ["scripts/demo/mock-keeperhub-server.mjs"], {
    ...
    setTimeout(() => reject(new Error(`mock did not start. stdout=${stdout} stderr=${stderr}`)), 5000);
  });
}
...
async function testProviderRoundTrip(): Promise<void> {
  ...
  const result = await provider.executeTransfer({
    ...
    from: "0xFromAddress0000000000000000000000000000000000",
    to: "0xToAddress0000000000000000000000000000000000",
  ...
  assert(
    result.txHash.startsWith("0xMOCK"),
    `expected mock txHash, got ${result.txHash}`,
  );
  assert(
    result.from === "0xFromAddress0000000000000000000000000000000000",
    `expected from to round-trip, got ${result.from}`,
  );
...
async function testAgentMethodShape(): Promise<void> {
  ...
  try {
    ...
    assert(
      ...
      `expected status=approved, got ${result.status}`,
    );
    assert(
    ...
    assert(
      result.runId === "run-1234",
      `expected runId=run-1234, got ${result.runId}`,
    );
    assert(
...
async function testAgentMethodHeld(): Promise<void> {
  ...
  try {
    ...
    assert(
      result.status === "held",
      `expected status=held, got ${result.status}`,
    );
    assert(
