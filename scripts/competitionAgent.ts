...
async function startCompetitionAgent() {
  ...
  try {
    ...
    console.log(
      "🔄 Making trades every 2 hours to meet 3+ trades/day requirement"
    );
    console.log(
      "📈 Monitor your performance at: https://competitions.recall.network\n"
    );
    ...
    const tradingInterval = setInterval(
        ...
        try {
          ...
          if (Date.now() - dayStartTime > oneDayMs) {
            ...
            if (tradesPerDay < config.minTradesPerDay) {
              console.warn(
                `⚠️  Warning: Only made ${tradesPerDay} trades yesterday (minimum: ${config.minTradesPerDay})`
              );
            }
            tradesPerDay = 0;
          ...
          if (result) {
            ...
            if (result.execution?.success) {
              ...
              console.log(
                `❌ Trade execution failed: ${result.execution.transaction.error}`
              );
            }

            // Log governance compliance
            const violations = result.policyChecks.filter(
              (check) => !check.passed
            );
            if (violations.length > 0) {
              console.log(`🚨 Policy violations: ${violations.length}`);
              violations.forEach((v) =>
                console.log(`   - ${v.policyId}: ${v.reason}`)
              );
            }

            console.log(
              `📊 Daily progress: ${tradesPerDay}/${config.minTradesPerDay} trades`
            );
          }
        } catch (error) {
      ...
      4 * 60 * 60 * 1000
    ); // Every 4 hours (6 trades per day)
    ...
    const statusInterval = setInterval(
      () => {
        ...
        console.log(
          `   Today's trades: ${tradesPerDay}/${config.minTradesPerDay}`
        );
        console.log(
    ...
    process.on("SIGINT", async () => {
      ...
      console.log(
        `   Check your ranking at: https://competitions.recall.network`
      );
