...
async function deployContracts() {
  try {
    if (!process.env.FILECOIN_RPC_URL || !process.env.FILECOIN_PRIVATE_KEY || !process.env.USDFC_TOKEN_ADDRESS) {
      throw new Error('Missing required environment variables for deployment. Please set FILECOIN_RPC_URL, FILECOIN_PRIVATE_KEY, and USDFC_TOKEN_ADDRESS in your .env file.');
    }
    ...
    const GovernanceContract = await ethers.getContractFactory('GovernanceContract', wallet);
    ...
    // Deploy StorageContract (with USDFC token address)
    logger.info('Deploying StorageContract...');
    const StorageContract = await ethers.getContractFactory('StorageContract', wallet);
    const storageContract = await StorageContract.deploy(process.env.USDFC_TOKEN_ADDRESS);
    await storageContract.waitForDeployment();
    const storageAddress = await storageContract.getAddress();
    logger.info(`StorageContract deployed at: ${storageAddress}`);

    // Create a sample policy
    logger.info('Creating sample governance policy...');
    const policyId = ethers.keccak256(ethers.toUtf8Bytes('sample-policy-' + Date.now()));
    ...
    const activatePolicyTx = await governanceContract.updatePolicyStatus(policyId, 1); // Active
    ...
    const agentId = ethers.keccak256(ethers.toUtf8Bytes('sample-agent-' + Date.now()));
    ...
    logger.info(`Governance stats - Policies: ${stats[0]}, Agents: ${stats[1]}, Actions: ${stats[2]}`);

    const storageStats = await storageContract.getStats();
    logger.info(`Storage stats - Storage Requests: ${storageStats[0]}, Retrieval Requests: ${storageStats[1]}, Providers: ${storageStats[2]}`);
