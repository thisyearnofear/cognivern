...
`pnpm lint` is the CI lint command and should also be run for frontend changes. It currently reports pre-existing repository-wide formatting debt, so record its result rather than assuming a clean pass.
