...
describe("workspaceRateLimit", () => {
  ...
  it("blocks requests over the limit", () => {
    ...
    limiter(makeReq({ headers: { "x-workspace-id": wsId } }), res, next);
    limiter(makeReq({ headers: { "x-workspace-id": wsId } }), res, next);
    limiter(makeReq({ headers: { "x-workspace-id": wsId } }), res, next);
    limiter(makeReq({ headers: { "x-workspace-id": wsId } }), res, next);
  ...
  it("uses workspace ID from header", () => {
...
describe("apiKeyRateLimit", () => {
  ...
  it("passes through when no API key is present", () => {
  ...
  it("blocks requests over the limit for the same key", () => {
    const limiter = apiKeyRateLimit(2, 60_000);
    const next = vi.fn();
    const res = mockRes();

    limiter(mockReq({ "x-api-key": "sk-abc123" }), res, next);
    limiter(mockReq({ "x-api-key": "sk-abc123" }), res, next);
    limiter(mockReq({ "x-api-key": "sk-abc123" }), res, next);
  ...
  it("tracks different API keys independently", () => {
    ...
    limiter(mockReq({ "x-api-key": "sk-key1" }), res, next);
    limiter(mockReq({ "x-api-key": "sk-key2" }), res, next);
    limiter(mockReq({ "x-api-key": "sk-key1" }), res, next);
